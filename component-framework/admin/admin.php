...
function oxygen_vsb_after_post_title() {
...
add_action("edit_form_after_title", "oxygen_vsb_after_post_title");
...
function oxygen_vsb_get_post_builder_link($post_id) {
...
function oxygen_vsb_hide_admin_bar() {

    if ( defined("OXYGEN_VSB_SHOW_BUILDER") ) {
    	add_filter('show_admin_bar', '__return_false');
    }
}
add_action('init','oxygen_vsb_hide_admin_bar');
...
function oxygen_vsb_enqueue_admin_scripts( $hook ) {

	// load css on all pages
	wp_enqueue_style ( 'ct-admin-style', OXYGEN_VSB_FW_URI . "/admin/admin.css" );
    ...
    if ( $screen->post_type == "ct_template" ) {
        wp_enqueue_script( 'select2', OXYGEN_VSB_FW_URI . "/vendor/select2/select2.full.min.js", array( 'jquery' ) );
    	wp_enqueue_style ( 'select2', OXYGEN_VSB_FW_URI . "/vendor/select2/select2.min.css" );
    }

    wp_enqueue_script( 'ct-admin-script', OXYGEN_VSB_FW_URI . "/admin/admin.js" );
}
add_action( 'admin_enqueue_scripts', 'oxygen_vsb_enqueue_admin_scripts' );
...
function oxygen_vsb_shortcodes_save_meta_box( $post_id ) {
	...
	$components = oxygen_vsb_parse_shortcodes( $shortcodes, true, false );
	$shortcodes = oxygen_vsb_parse_components_tree( $components['content'] );

	update_post_meta( $post_id, 'ct_builder_shortcodes', $shortcodes );
}
add_action( 'save_post', 'oxygen_vsb_shortcodes_save_meta_box' );