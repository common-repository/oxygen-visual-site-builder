...
add_action( 'init', 'oxygen_vsb_add_templates_cpt' );

function oxygen_vsb_add_templates_cpt() {
...
add_filter( 'get_sample_permalink_html', 	'oxygen_vsb_template_remove_permalink' );
//add_filter( 'pre_get_shortlink', 			'oxygen_vsb_template_remove_shortlink', 10, 2 );

function oxygen_vsb_template_remove_permalink( $return ) {
	global $post;
    return 'ct_template' === get_post_type( $post->ID ) ? '' : $return;
}

function oxygen_vsb_template_remove_shortlink( $false, $post_id ) {
	global $post;
...
add_action( 'template_redirect', 'oxygen_vsb_check_templates_post');

function oxygen_vsb_check_templates_post() {
	global $post;
	...
	if ( $post_type == 'ct_template' && !defined("OXYGEN_VSB_SHOW_BUILDER") 
...
add_action( 'add_meta_boxes', 'oxygen_vsb_template_meta_box' );

add_action( 'save_post', 'oxygen_vsb_template_save_meta_box' );

function oxygen_vsb_template_meta_box() {
	...
	add_meta_box(
		...
		'oxygen_vsb_template_meta_box_callback',
		'ct_template',
		'normal',
		'high'
	);
...
function oxygen_vsb_template_meta_box_callback( $post ) {
...
function oxygen_vsb_template_save_meta_box( $post_id ) {
...
//$sanitizedValues = array_filter($_POST['ct_template_archive_post_types'], 'ctype_digit');
	...
	$template_single_all 	= sanitize_text_field( $_POST['ct_template_single_all'] );
	
	// post types
	$template_post_types 	= is_array($_POST['ct_template_post_types']) ? array_map('sanitize_text_field', $_POST['ct_template_post_types']): array();
	...
	// taxonomies
	$use_taxonomies 		= sanitize_text_field( $_POST['ct_use_template_taxonomies'] );
	$taxonomy_names 		= is_array($_POST['ct_taxonomy_names']) ? array_map('sanitize_text_field', $_POST['ct_taxonomy_names']): array();
	$taxonomy_values 		= is_array($_POST['ct_taxonomy_values']) ? array_map('sanitize_text_field', $_POST['ct_taxonomy_values']): array();
...
function oxygen_vsb_custom_views_columns($columns) {
...
function oxygen_vsb_custom_view_column( $column, $post_id ) {
    switch ( $column ) {
...
add_filter( 'manage_ct_template_posts_columns', 'oxygen_vsb_custom_views_columns' );
add_action( 'manage_ct_template_posts_custom_column' , 'oxygen_vsb_custom_view_column', 10, 2 );
...
function oxygen_vsb_oxygen_meta_box() {
	...
	foreach ( $post_types as $post_type ) {
		...
		add_meta_box(
			...
			'oxygen_vsb_view_meta_box_callback',
			$post_type->name,
			'advanced',
			'high'
		);
...
add_action( 'add_meta_boxes', 'oxygen_vsb_oxygen_meta_box' );
...
function oxygen_vsb_view_meta_box_callback( $post ) {
	global $wpdb;
	// Add a nonce field so we can check for it later
	...
	if ($screen->post_type != "ct_template") {
		...
		if ( get_option( 'page_for_posts' ) == $post->ID || get_option( 'page_on_front' ) == $post->ID ) {
			$generic_view = oxygen_vsb_get_archives_template( $post->ID );
		}
		else {
			$generic_view = oxygen_vsb_get_posts_template( $post->ID );
		}
		...
		<ul>
			<li>
				...
				<div class="ct_template_option_panel" <?php echo (!$custom_view || $ct_render_post_using == 'other_template')?'style="display:none;"':'';?>>
					...
					if ( $custom_view ) { ?>
							...
							<a href="<?php echo esc_url(oxygen_vsb_get_post_builder_link( $post->ID )); ?>" class="button button-primary" style="vertical-align:middle">
			...
			<li>
				...
				<div class="ct_template_option_panel" <?php echo (($ct_render_post_using == false && $custom_view) || ($ct_render_post_using == 'custom_template' && $custom_view))?'style="display:none;"':'';?> >
						...
						<label style="margin-top: 10px;"><?php _e("Choose Template", "component-theme");?>
							...
							<select name="ct_other_template">
								<?php
									...
									if($generic_view) {
									?>
									<option value="0" <?php echo intval($ct_other_template) == $generic_view->id?'selected':'';?>>Default (<?php echo sanitize_text_field( $generic_view->post_title ); ?>)</option>
									<?php
									}
									else {
										?>
									...
									foreach($templates as $template) {
										...
										if(!($ct_template_type && $ct_template_type =='reusable_part')) {
											?>
											<option value="<?php echo intval( $template->id ); ?>" <?php echo intval($ct_other_template) == $template->id?'selected':'';?>><?php echo sanitize_text_field( $template->post_title ); ?></option>
											<?php
										}
									}
							...
							<div class="ct-admin-tooltip">
								...
								<div>
									...
									<a href='<?php echo esc_url( get_edit_post_link( $generic_view->ID ) );?>'>
					...
					?>
					<br />
					<a href="<?php echo esc_url(oxygen_vsb_get_post_builder_link( $def_temp_id )); ?>" class="button button-primary" style="margin-top: 8px;">
							...
							*/
							...
							?>
								<p><span class="dashicons dashicons-info"></span>
									You can create templates and mass apply them to content matching any criteria from Oxygen-><a href='<?php echo esc_url( admin_url('edit.php?post_type=ct_template') );?>'>Templates</a>
								</p>
	...
	// Button only for "Views"
		...
		<p><a href="<?php echo esc_url(oxygen_vsb_get_post_builder_link( $post->ID )); ?>" class="button button-primary">
			<?php printf( __("Edit '%s' in Visual Editor", "component-theme"), sanitize_text_field( get_the_title() ) ); ?>
		</a></p>
...
function oxygen_vsb_view_save_meta_box( $post_id ) {
	...
	if (isset($_POST["ct_redirect_inner_content"]) && $_POST["ct_redirect_inner_content"] == "true") {
		...
		*/
		// redirect to builder
		wp_redirect( esc_url_raw( oxygen_vsb_get_post_builder_link( $post_id ).'&ct_inner=true' ) );
		exit;
	}
	...
	if (isset($_POST["ct_custom_view_on_create_copy"]) && $_POST["ct_custom_view_on_create_copy"] == "true") {
		...
		// redirect to builder
		wp_redirect( esc_url_raw( oxygen_vsb_get_post_builder_link( $post_id ) ) );
		exit;
	}
...
add_action( 'save_post', 'oxygen_vsb_view_save_meta_box' );
...
function oxygen_vsb_view_order_meta_box() {
	if(!current_user_can( 'manage_options' )) {
		return;
	}
	add_meta_box(
		...
		'oxygen_vsb_view_order_meta_box_callback',
		'ct_template',
		'side'
	);
}

add_action( 'add_meta_boxes', 'oxygen_vsb_view_order_meta_box' );

add_action( 'save_post', 'oxygen_vsb_view_order_save_meta_box' );
...
function oxygen_vsb_view_order_meta_box_callback( $post ) {
...
function oxygen_vsb_view_order_save_meta_box( $post_id ) {
...
add_action( 'restrict_manage_posts', 'oxygen_vsb_views_filter_dropdown' );
function oxygen_vsb_views_filter_dropdown() {
...
function oxygen_vsb_views_filter( $query )
{
    global $typenow;
    global $pagenow;
...
add_filter( 'parse_query', 'oxygen_vsb_views_filter' );
...
function oxygen_vsb_views_order_column( $columns ) {
...
add_filter( 'manage_ct_template_posts_columns', 'oxygen_vsb_views_order_column' );
...
function oxygen_vsb_views_order_value( $column, $post_id ) {
    switch ( $column ) {
...
add_action( 'manage_ct_template_posts_custom_column' , 'oxygen_vsb_views_order_value', 10, 2 );
...
function oxygen_vsb_views_order_sortable( $columns ) {
   
    $columns['ct_view_order'] = 'ct_view_order';

    return $columns;
}
add_filter( 'manage_edit-ct_template_sortable_columns', 'oxygen_vsb_views_order_sortable' );
...
function oxygen_vsb_views_order_sort( $query ) {
...
add_action( 'pre_get_posts', 'oxygen_vsb_views_order_sort' );
...
function oxygen_vsb_views_type_column( $columns ) {
...
add_filter( 'manage_ct_template_posts_columns', 'oxygen_vsb_views_type_column' );
...
function oxygen_vsb_views_type_value( $column, $post_id ) {
	$post_types = '';
    switch ( $column ) {
...
add_action( 'manage_ct_template_posts_custom_column' , 'oxygen_vsb_views_type_value', 10, 2 );
...
function oxygen_vsb_views_taxonomies_column( $columns ) {
...
add_filter( 'manage_ct_template_posts_columns', 'oxygen_vsb_views_taxonomies_column' );


function oxygen_vsb_views_taxonomies_value( $column, $post_id ) {
    switch ( $column ) {
...
add_action( 'manage_ct_template_posts_custom_column' , 'oxygen_vsb_views_taxonomies_value', 10, 2 );