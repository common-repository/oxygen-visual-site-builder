...
CTFrontendBuilder.controller("ControllerTemplates", function($scope, $timeout){
	...
	$scope.addWidget = function(className, idBase, prettyName) {
		var content = "<div id='ct-simple-modal-content' class='upgrade_modal_class'><h1>Modal Heading</h1>" +
			"<p>Please upgrade to the pro version of Oxygen to embed widgets</p>" +
			"<div><a href='http://oxygenapp.com' target='_blank'>Get Oxygen Pro</a> <a href='#' id='ct-simple-modal-close'>Close</a></div></div>";

		$scope.showUpgradeModal(content);
	}

	$scope.addItem = function () {
		var content = "<div id='ct-simple-modal-content' class='upgrade_modal_class'><h1>Modal Heading</h1>" +
			"<p>Please upgrade to the pro version of Oxygen to access our library of pre-built components and page designs.</p>" +
			"<div><a href='http://oxygenapp.com' target='_blank'>Get Oxygen Pro</a> <a href='#' id='ct-simple-modal-close'>Close</a></div></div>";
	...
	$scope.showUpgradeModal = function(content) {
		if(typeof(content) === 'undefined')
			return;

		var modal = angular.element('#ct-simple-modal');
		if(modal.length < 1) {
			modal = angular.element('<div>').attr('id', 'ct-simple-modal');
		} else {
			modal.html('');
		}
		...
		angular.element('#ct-simple-modal-close').off('click');
		angular.element('#ct-simple-modal-close').on('click', function(e) {
			e.preventDefault();
			modal.remove();
		});
