...
Class OXYGEN_VSB_Code_Block extends OXYGEN_VSB_Component {
	...
	function __construct( $options ) {
		...
		add_filter( 'template_include', array( $this, 'oxygen_vsb_code_block_single_template'), 100 );

		add_action("oxygen_vsb_toolbar_component_settings", array( $this, "code_editor_button") );
		...
		if(isset($_REQUEST['action']) && stripslashes($_REQUEST['action']) == 'ct_exec_code') {
			...
			add_action( 'wp', array( $this, 'oxygen_vsb_code_remove_template_redirect'));
		}
	}

	function oxygen_vsb_code_remove_template_redirect() {
		global $wp_filter;
	...
	function oxygen_vsb_code_block_single_template( $template ) {
...
// Create instance
$html = new OXYGEN_VSB_Code_Block( array( 
			'name' 		=> 'Code Block',
			'tag' 		=> 'ct_code_block',
			'params' 	=> array(
					array(
