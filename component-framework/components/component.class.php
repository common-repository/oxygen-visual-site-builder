...
Class OXYGEN_VSB_Component {
	...
	function init( $options ) {
		...
		if ( ! isset( $_GET['ct_builder'] ) || ! $_GET['ct_builder'] ) {
			add_action( "oxygen_vsb_footer_styles", array( $this, 'output_css' ) );
		}
		...
		add_action("oxygen_vsb_toolbar_fundamentals_list", 		array( $this, "component_button") );
		add_action("oxygen_vsb_toolbar_component_header", 		array( $this, "component_header") );
		add_action("oxygen_vsb_toolbar_component_settings", 	array( $this, "component_settings") );
		
		add_filter("oxygen_vsb_component_default_params",  		array( $this, "init_default_params") );
		add_filter("oxygen_vsb_not_css_options",  				array( $this, "not_css_options") );
		add_filter("oxygen_vsb_components_nice_names",  		array( $this, "component_nice_name") );
		add_filter("oxygen_vsb_filter_{$options['tag']}",       array( $this, "filter_component" ) );
	}
	...
	function ng_attributes( $param_name, $attributes = "class,model,change") { 
		...
		if ( in_array('class-fake', $attributes) ) { ?>
			ng-class="checkOptionChanged(component.active.id,'<?php echo esc_attr( $param_name ); ?>')"
		<?php }

		if ( in_array('model', $attributes) ) { ?>
			ng-model="component.options[component.active.id]['model']['<?php echo esc_attr( $param_name ); ?>']"
			ng-model-options="{ debounce: 10 }"
		<?php }

		if ( in_array('change', $attributes) ) { ?>
			ng-change="setOption(component.active.id,'<?php echo isset($this->options['tag'])? esc_attr( $this->options['tag'] ):''; ?>','<?php echo isset($param_name)? esc_attr( $param_name ):''; ?>'<?php echo isset($shortcode_arg)?$shortcode_arg:''; ?>)"
		<?php }
	...
	function component_header() { 
		...
		<span ng-if="isActiveName('<?php echo esc_attr( $this->options['tag'] ); ?>')">
			...
			<?php foreach ( $this->options['params'] as $param ) : 
				...
				$ct_class = "ct-" . (isset($this->options['tag'])? esc_attr( $this->options['tag'] ):'') . "-" . (isset($param['param_name'])? esc_attr( $param['param_name'] ):'');
			?>
			<div class="ct-toolitem <?php echo esc_attr( $ct_class ); ?> <?php if($param['type']=='typography') echo 'ct-fontsdropdown'; ?>" <?php echo $ng_show; ?>>
				<h3><?php echo sanitize_text_field( $param['heading'] ); ?></h3>
				<?php switch ( $param['type'] ) {
					...
					case 'mediaurl' : ?>
						...
						<div class="ct-textbox ct-textbox-fg ct-textbox-browse <?php if ( $param['css'] === false ) echo "oxygen-special-property"; ?>">
							...
							<input type="text" spellcheck="false"
								ng-class="isInherited(component.active.id, '<?php echo esc_attr( $param['param_name'] ); ?>')"
								<?php $this->ng_attributes($param['param_name']); ?>/>
						</div>
						<?php break;
					
					case 'textfield' : ?>
						<div class="ct-textbox <?php if ( isset ( $param["class"] ) ) echo esc_attr( $param["class"] );?>">
					...
					case 'columnwidth' : ?>
						...
						<div class="ct-measurebox-container clearfix"
							ng-class="{'ct-word-selected':getOptionUnit('<?php echo esc_attr( $param['param_name'] ); ?>')=='auto'}">
							<div class="ct-measurebox-wrap">
								<div class="ct-measurebox ct-column-width"
									...
									<input class="ct-measure-value ct-number-font <?php if ( $param['css'] === false ) echo "oxygen-special-property"; ?>" type="text" spellcheck="false"
										ng-change="setOption(component.active.id,'<?php echo esc_attr( $this->options['tag'] ); ?>','<?php echo esc_attr( $param['param_name'] ); ?>'); updateColumnsOnChange(component.active.id,{{component.options[component.active.id]['model']['width']}})"
										<?php $this->ng_attributes($param['param_name'],"class,model"); ?>/>
									<div class="ct-measure-type">{{getOptionUnit('<?php echo esc_attr( $param['param_name'] ); ?>')}}</div>
								</div>
							</div>
						</div>
						<?php break;
					...
					case 'measurebox' : ?>
						...
						<div class="ct-measurebox-container clearfix"
							...
							<div class="ct-measurebox-wrap <?php if ( isset($param['css']) && $param['css'] === false ) echo "oxygen-special-property"; ?>">
								...
								<?php OXYGEN_VSB_Toolbar::measure_type_select($param['param_name']); ?>
							</div>
						</div>
						...
						<div class="ct-selectbox">
							<ul class="ct-select <?php if ( $param['css'] === false ) echo "oxygen-special-property"; ?>">
								<li class="ct-selected"
									ng-class="isInherited(component.active.id, '<?php echo esc_attr( $param['param_name'] ); ?>')">
										{{getOption('<?php echo esc_attr( $param['param_name'] ); ?>')}}<span class="ct-icon ct-dropdown-icon"></span>
								</li>
								<li>
									<ul class="ct-dropdown-list">
										<?php foreach ( $param['value'] as $value => $name ) : ?>
											<li	ng-click="setOptionModel('<?php echo esc_attr( $param['param_name'] ); ?>','<?php echo esc_attr( $value ); ?>')">
					...
					case 'colorpicker' : ?>
						<div class="ct-colorpicker"
							...
							<input class="ct-color"
								...
								<?php $this->ng_attributes($param['param_name']); ?>
								ng-style="{'background-color':getOption('<?php echo esc_attr( $param['param_name'] ); ?>')}"/>
						</div>
						<?php break;
						...
						<div class="ct-selectbox ct-heading-type-select">
							<ul class="ct-select oxygen-special-property">
								...
								<li>
									<ul class="ct-dropdown-list">
										<?php foreach ( $param['value'] as $value => $name ) : ?>
										<li ng-click="setOptionModel('<?php echo esc_attr( $param['param_name'] ); ?>','<?php echo esc_attr( $value ); ?>');changeTag(<?php if ($this->options['tag']=='ct_shortcode') echo "'shortcode'";?>)">
											<?php echo sanitize_text_field( $name ); ?>
										</li>
										<?php endforeach; ?>
									</ul>
					...
					case 'align' : ?>
						<div class="ct-sml-button-list">
							<ul>
								<li ng-class="{'ct-active':component.options[component.active.id]['model']['<?php echo esc_attr( $param['param_name'] ); ?>']=='left'}">
									<label title="<?php _e( 'Justify Left', 'component-theme' ); ?>" class="clearfix">
										<input type="radio" <?php $this->ng_attributes($param['param_name']); ?> 
											name="<?php echo esc_attr( $this->options['tag'] ."_". $param['param_name'] ); ?>"
											value="left"
											ng-click="radioButtonClick('<?php echo esc_attr( $this->options['tag'] ); ?>', '<?php echo esc_attr( $param['param_name'] ); ?>', 'left');">
										<span class="ct-icon ct-paragraph-left-icon"></span></label>
								</li>
								<li ng-class="{'ct-active':component.options[component.active.id]['model']['<?php echo esc_attr( $param['param_name'] );?>']=='center'}">
									<label title="<?php _e( 'Justify Center', 'component-theme' ); ?>" class="clearfix">
										<input type="radio" <?php $this->ng_attributes($param['param_name']); ?> 
											name="<?php echo esc_attr( $this->options['tag'] ."_". $param['param_name'] ); ?>"
											value="center"
											ng-click="radioButtonClick('<?php echo esc_attr( $this->options['tag'] ); ?>', '<?php echo esc_attr( $param['param_name'] ); ?>', 'center');">
										<span class="ct-icon ct-paragraph-center-icon"></span></label>
								<li ng-class="{'ct-active':component.options[component.active.id]['model']['<?php echo esc_attr( $param['param_name'] );?>']=='right'}">
									<label title="<?php _e( 'Justify Right', 'component-theme' ); ?>" class="clearfix">
										<input type="radio" <?php $this->ng_attributes($param['param_name']); ?> 
											name="<?php echo esc_attr( $this->options['tag'] ."_". $param['param_name'] ); ?>"
											value="right"
											ng-click="radioButtonClick('<?php echo esc_attr( $this->options['tag'] ); ?>', '<?php echo esc_attr( $param['param_name'] ); ?>', 'right');">
					...
					default : ?>
						<span><?php printf( __( 'Wrong parameter type: %s', 'component-theme' ), esc_attr( $param['type'] ) ); ?></span>
						<?php break;
				} ?>
			</div><!-- /.ct-toolitem -->
			<?php endforeach; ?>
	...
	function component_settings() { ?>
		...
		<span ng-if="isActiveName('<?php echo esc_attr( $this->options['tag'] ); ?>')">
			...
			<?php if (  isset($this->options['content_editable']) && $this->options['content_editable'] &&
				...
				<?php if ( defined( "OXYGEN_VSB_TEMPLATE_EDIT" ) ) : ?>
					<span title="<?php _e( 'Data', 'component-theme' ); ?>" 
						ng-show="isActiveName('<?php echo esc_attr( $this->options['tag'] ); ?>')"
	 ...
	 * via add_filter("oxygen_vsb_not_css_options")
	 *
	 * @since 0.3.2
	 */
	...
	function set_options( $atts ) {
		...
		foreach($states as $key => $state) {
			...
			elseif(Oxygen_VSB_Base::is_pseudo_element($key)) {
	...
	function build_css($states, $selector, $is_media = false) {
		...
		// get page settings
		$page_settings = Oxygen_VSB_Base::oxygen_vsb_get_page_settings( get_the_ID() );
		...
		if ( isset($this->custom_js) && is_array( $this->custom_js ) ) {
			...
			foreach ( $this->custom_js as $component_id => $custom_js ) {
				...
				if ( ! defined("OXYGEN_VSB_SHOW_BUILDER") ) {
					...
					echo "<script type=\"text/javascript\" id=\"ct_custom_js_".esc_attr($component_id)."\">";
						echo $code;
					echo "</script>\r\n";
				}
			}
...
// End OXYGEN_VSB_Component class	
}