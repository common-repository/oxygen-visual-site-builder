...
if ( ! $styles ) {
	...
	$styles = ob_get_clean();
	$styles = Oxygen_VSB_Base::oxygen_css_minify( $styles );
	...
	// save styles cache
	//update_post_meta( $oxygen_vsb_template_id, "oxygen_page_styles", $styles );
	//update_post_meta( $oxygen_vsb_template_id, "oxygen_shortcodes_css_rendered_timestamp", time() );
}
	...
	foreach ( $selectors as $selector => $states ) {
		foreach ( $states as $state => $options ) {
			...
			if ( $state == 'media' ) {
				...
				foreach ( $media_queries_list as $media_name => $media ) {
					...
					if ( $options[$media_name] && $media_name != "default") {
						...
						foreach ( $options[$media_name] as $media_state => $media_options ) {
							$css .= Oxygen_VSB_Base::oxygen_vsb_generate_class_states_css($selector, $media_state, $media_options, true, true);
						}
						$css .= "}\n\n";
					}
				}
				...
				$css = Oxygen_VSB_Base::oxygen_vsb_generate_class_states_css($selector, $state, $options, false, true).$css;
			}
		}
	}
}
echo Oxygen_VSB_Base::oxygen_css_minify($css);