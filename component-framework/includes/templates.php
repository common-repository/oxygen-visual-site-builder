...
function oxygen_vsb_template_output( $as_shortcodes = false ) {
	
	$shortcodes = Oxygen_VSB_Base::oxygen_vsb_template_shortcodes(true);
	...
	global $oxygen_vsb_template_id;
	$oxygen_vsb_template_id = ( isset($template) && isset($template->ID) ) ? $template->ID : get_the_ID();
	
	/*if ( ! defined("OXYGEN_VSB_SHOW_BUILDER") ) {
		$shortcodes_time 	= get_post_meta( $oxygen_vsb_template_id, "oxygen_shortcodes_css_rendered_timestamp", true );
		$stylesheets_time 	= get_option( "oxygen_stylesheets_css_rendered_timestamp", true );

		//wp_enqueue_style("oxygen-styles", Oxygen_VSB_Base::oxygen_vsb_get_current_url( 'xlink=css&tid='.$oxygen_vsb_template_id.'&shortcodes_time='.$shortcodes_time.'&stylesheets_time='.$stylesheets_time ), array('oxygen') );

		wp_enqueue_style("oxygen-styles", Oxygen_VSB_Base::oxygen_vsb_get_current_url( 'xlink=css' ), array('oxygen') );
	}*/
	...
	if ( $shortcodes ) {
		...
		if ( !isset( $_REQUEST['xlink'] ) && isset( $template ) ) {
			echo "<!-- Oxygen Template ID: " . esc_html( $template->post_title ) . " (" . esc_html( $template->ID ) . ") -->";
		}
...
function oxygen_vsb_templates_buffer_start() { 
	...
	if ( $template_content !== false ) {
		...
		if(!isset($oxygen_vsb_replace_render_template) || 
			$oxygen_vsb_replace_render_template == get_single_template() ||
			$oxygen_vsb_replace_render_template == get_page_template() ||
			$oxygen_vsb_replace_render_template == get_index_template()) {
...
function oxygen_vsb_templates_buffer_end() { 
...
add_action('oxygen_vsb_builder_start', 	'oxygen_vsb_templates_buffer_start');
add_action('oxygen_vsb_builder_end', 	'oxygen_vsb_templates_buffer_end');
...
function oxygen_vsb_template_builder_settings() { 

	// show only for templates
	if ( !defined("OXYGEN_VSB_TEMPLATE_EDIT") || OXYGEN_VSB_TEMPLATE_EDIT !== true ) {
		return false;
	}
...
add_action("oxygen_vsb_toolbar_component_settings", "oxygen_vsb_template_builder_settings" );
...
function oxygen_vsb_get_posts_template( $post_id ) {
...
function oxygen_vsb_get_archives_template( $post_id = false ) {
	...
	foreach ( $templates->posts as $template ) {
		...
		if ( is_tax() ) {
			...
			$term_id 	= get_queried_objeoxygen_vsb_id();
...
function oxygen_vsb_get_templates_post( $template_id, $current_post_id = false, $option = false ) {
	...
	if ( !$current_post_id ) {
		...
		foreach ( $posts as $post ) {
			if ( $post->ID == $current_post_id ) {
				...
				// update some values
				$filtered_post = oxygen_vsb_filter_post_object( $post, true ); // since its a template preivew, we dont want anything to do with the post's custom view
...
function oxygen_vsb_get_templates_term( $template_id, $term_id = false, $option = false ) {
	...
	if ( ! $terms[$new_term_key] ) {
		...
		oxygen_vsb_add_term_posts( $terms, 'post_types', $post_types );
		oxygen_vsb_add_term_posts( $terms, 'category', $categories );
		oxygen_vsb_add_term_posts( $terms, 'post_tag', $tags );
		oxygen_vsb_add_term_posts( $terms, 'authors', $authors );
		...
		if ( is_array( $taxonomies ) ) {
			...
			foreach ( $terms_names as $name => $ids ) {
				oxygen_vsb_add_term_posts( $terms, $name, $ids );
			}
		}
		...
		if ( $template_index ) {
			oxygen_vsb_add_term_posts( $terms, "index");
		}
		if ( $template_front_page ) {
			oxygen_vsb_add_term_posts( $terms, "front_page");
		}
		if ( $template_date_archive ) {
			oxygen_vsb_add_term_posts( $terms, "date_archive");
		}
		if ( $template_blog_posts ) {
			oxygen_vsb_add_term_posts( $terms, "blog_posts");
		}
		if ( $template_search_page ) {
			oxygen_vsb_add_term_posts( $terms, "search_page");
		}
		if ( $template_404_page ) {
			oxygen_vsb_add_term_posts( $terms, "404_page");
		}
		...
		foreach ( $terms as $term_key => $term ) {
			if ( is_array( $term["term_posts"] ) ) {
				foreach ( $term["term_posts"] as $post_key => $post ) {
					$terms[$term_key]["term_posts"][$post_key] = oxygen_vsb_filter_post_object( $post );
				}
			}
		}
...
function oxygen_vsb_add_term_posts( &$terms, $taxonomy_name, $term_ids = false ) {
...
function oxygen_vsb_filter_post_object( $post_object, $no_custom_view = false ) {
	...
	$tree 		= oxygen_vsb_parse_shortcodes($shortcodes);
...
function oxygen_vsb_get_template_header_footer( $name ) {
	...
	if ( $template ) {
		...
		global $oxygen_vsb_template_header;
		global $oxygen_vsb_template_footer;
		global $oxygen_vsb_template_header_footer_id;

		$oxygen_vsb_template_header = $header_footer[0];
		$oxygen_vsb_template_footer = $header_footer[1];
		$oxygen_vsb_template_header_footer_id = $template->ID;
	}

}
add_action( 'get_header', 'oxygen_vsb_get_template_header_footer' );
...
function oxygen_vsb_get_template_header() {

	// get footer code from global
	global $oxygen_vsb_template_header;

	if (!$oxygen_vsb_template_header)
		return;
	
	echo "<!-- CT Template Header Start-->\n";
	echo $oxygen_vsb_template_header;
	echo "\n<!-- CT Template Header End-->";
}
function oxygen_vsb_get_template_footer() {

	// get footer code from global
	global $oxygen_vsb_template_footer;

	if (!$oxygen_vsb_template_footer)
		return;
	
	echo "<!-- CT Template Footer Start-->\n";
	echo $oxygen_vsb_template_footer;
	echo "\n<!-- CT Template Footer End-->";
}

add_action( 'oxygen_vsb_builder_start', 'oxygen_vsb_get_template_header' );
add_action( 'oxygen_vsb_builder_end', 'oxygen_vsb_get_template_footer' );