...
Class OXYGEN_VSB_Toolbar {

	function oxygen_vsb_Toolbar() {
	...
	function toolbar_init() {
		...
		if ( current_user_can("edit_posts") && defined("OXYGEN_VSB_SHOW_BUILDER") ) {
			add_action("oxygen_vsb_before_builder", array( $this, "toolbar_view") );
		}
		...
		//$this->options['advanced'] = apply_filters("oxygen_vsb_component_advanced_options", $this->options['advanced']);

		// include styles
		add_action("wp_enqueue_scripts", array( $this, "enqueue_scripts" ) );

		add_action("oxygen_vsb_builder_ng_init", array( $this, "init_folders" ) );

		// output main toolbar elements
		add_action("oxygen_vsb_toolbar_component_header",			array( $this, "component_header"), 1 );
		add_action("oxygen_vsb_toolbar_advanced_settings", 			array( $this, "advanced_settings") );
		add_action("oxygen_vsb_toolbar_advanced_anchors", 			array( $this, "advanced_settings_anchors") );
		add_action("oxygen_vsb_toolbar_advanced_child_anchors", 	array( $this, "advanced_settings_child_anchors") );

		add_action("oxygen_vsb_toolbar_code_editor_settings", 		array( $this, "code_editor_settings") );
		add_action("oxygen_vsb_toolbar_code_editor_anchors", 		array( $this, "code_editor_settings_anchors") );

		add_action("oxygen_vsb_toolbar_components_list",			array( $this, "components_list") );
		add_action("oxygen_vsb_toolbar_components_anchors", 		array( $this, "components_anchors") );

		add_action("oxygen_vsb_toolbar_reusable_parts", 			array( $this, "oxygen_vsb_reusable_parts") );
		
		add_action("oxygen_vsb_toolbar_page_settings", 				array( $this, "oxygen_vsb_show_page_settings" ) );
		add_action("oxygen_vsb_toolbar_global_settings", 			array( $this, "oxygen_vsb_show_global_settings") );
		add_action("oxygen_vsb_dialog_window", 						array( $this, "dialog_window") );
	}
	...
	function enqueue_scripts() {
		...
		wp_enqueue_style ("ct-ui", 			OXYGEN_VSB_FW_URI . "/toolbar/UI/css/default.css");
		wp_enqueue_style ("ct-dom-treee", 	OXYGEN_VSB_FW_URI . "/toolbar/UI/css/domtree.css");
		//wp_enqueue_style ("ct-old-ui", 		OXYGEN_VSB_FW_URI . "/toolbar/toolbar.css");
	}
			...
			if ( isset($tab['children']) && $tab['children'] ) :
				...
				foreach ( $tab['children'] as $child_key => $child_tab ) : ?>
					...
					<div class="ct-panel clearfix <?php echo isset($classes)?$classes:''; ?> <?php echo $key.'-'.$child_key; ?>" ng-if="isShowChildTab('advanced', '<?php echo $key; ?>', '<?php echo $child_key; ?>')">
						<?php if ( file_exists( OXYGEN_VSB_FW_PATH . "/toolbar/views/$key/$key.$child_key.view.php" ) ) :
				...
				<div class="ct-panel clearfix <?php echo isset($classes)?$classes:''; ?> <?php echo $key ;?>" ng-if="isShowTab('advanced', '<?php echo $key; ?>')">
					<?php if ( file_exists( OXYGEN_VSB_FW_PATH . "/toolbar/views/$key.view.php" ) ) :
	...
	function oxygen_vsb_show_global_settings() { 
	...
	function output_folders_content( $folders, $main_key, $title, $path="" ) {
		...
		if ( $main_key && $path ) {
			$path .= " > " . "<span class=\"ct-folders-breadcrumb\" ng-click=\"openFolder('" . esc_attr( $main_key ) . "')\">" . esc_html( $title ) . "</span>";
		}
		elseif ( $main_key ) {
			$path .= "<span class=\"ct-folders-breadcrumb\" ng-click=\"openFolder('" . esc_attr( $main_key ) . "')\">" . esc_html( $title ) . "</span>";
		}
		...
		foreach ( $folders as $key => $folder ) : 
			...
			<div class="ct-panel ct-folder-<?php echo $slug; ?> <?php echo $folder_class; ?>" ng-if="isShowFolder('<?php echo $slug; ?>')">
				...
				<?php if ( isset($folder["id"]) && $folder["id"] === "widgets" ) : ?>
					<?php do_action("oxygen_vsb_toolbar_widgets_folder"); ?>
				<?php else : ?>

					<?php if ( isset($folder["name"]) && $folder["name"] === "WordPress" ) : ?>
						<?php do_action("oxygen_vsb_folder_component_shortcode"); ?>
					<?php endif; ?>
					...
					<?php if ( isset($folder["children"]) && $folder["children"] ) : ?>
						...
						?>
							...
							<?php if ( isset($subfolder["component"]) && $subfolder["component"] ) : ?>
								<?php do_action("oxygen_vsb_folder_component_" . $subslug); ?>
							<?php else : ?>
	...
	function components_list() { 
		...
		<div class="ct-panel" ng-if="isShowTab('components','fundamentals')">
			<?php do_action("oxygen_vsb_toolbar_fundamentals_list"); ?>
		</div>
		...
		<div class="ct-panel ct-reusable-parts" ng-if="isShowTab('components','reusable_parts')">
			<?php do_action("oxygen_vsb_toolbar_reusable_parts"); ?>
		</div>
	...
	function oxygen_vsb_reusable_parts() {
		...
		foreach ( $templates->posts as $template ) : ?>
			...
			<div class="ct-add-component-button">
				<div class="ct-reusable-title"><?php echo esc_html( $template->post_title ); ?></div>
				<div class="ct-add-component-icon" title="<?php _e("Add Re-usable part as single component", "component-theme")?>"
					ng-click="loadReusablePart(<?php echo esc_attr( $template->ID ); ?>)">
				...
				<div class="ct-add-component-icon" title="<?php _e("Add Re-usable part as editable fundamentals", "component-theme")?>"
					ng-click="loadReusablePart(<?php echo esc_attr( $template->ID ); ?>, component.active.id)">
	...
	function oxygen_vsb_show_page_settings() { ?>
	...
	static public function measure_type_select($option, $types = "px,%,em,auto") { 
		...
		<div class="ct-measure-type-select">
			...
			<div class="ct-button ct-measure-type-option"
				ng-click="setOptionUnit('<?php echo esc_attr( $option ); ?>', 'px')"
				ng-class="{'ct-active':getOptionUnit('<?php echo esc_attr( $option ); ?>')=='px'}">
				<span class="ct-bullet"></span> PX
			</div>
			<?php endif; ?>
			<?php if (in_array("%", $types)) : ?>
			<div class="ct-button ct-measure-type-option"
				ng-click="setOptionUnit('<?php echo esc_attr( $option ); ?>', '%')"
				ng-class="{'ct-active':getOptionUnit('<?php echo esc_attr( $option ); ?>')=='%'}">
				<span class="ct-bullet"></span> &#37;
			</div>
			<?php endif; ?>
			<?php if (in_array("em", $types)) : ?>
			<div class="ct-button ct-measure-type-option"
				ng-click="setOptionUnit('<?php echo esc_attr( $option ); ?>', 'em')"
				ng-class="{'ct-active':getOptionUnit('<?php echo esc_attr( $option ); ?>')=='em'}">
				<span class="ct-bullet"></span> EM
			</div>
			<?php endif; ?>
			<?php if (in_array("auto", $types)) : ?>
			<div class="ct-button ct-measure-type-option"
				ng-click="setOptionUnit('<?php echo esc_attr( $option ); ?>', 'auto')"
				ng-class="{'ct-active':getOptionUnit('<?php echo esc_attr( $option ); ?>')=='auto'}">
	...
	function measure_box_options( $option, $opposite_option, $text = "" ) { ?>
		...
		<div class="ct-measurebox-options">
			<div class="ct-checkbox">
				<label><input class="ct-apply-opposite-trigger" type="radio" name="<?php echo esc_attr( $option ); ?>_measure"
					data-option="<?php echo esc_attr( $option ); ?>"
					data-opposite-option="<?php echo esc_attr( $opposite_option ); ?>"/>
					<span class="ct-checkbox-box"></span><span><?php echo esc_html( $text ) ?></span></label>
			</div>
			<div class="ct-checkbox">
				<label><input class="ct-apply-all-trigger" type="radio" name="<?php echo esc_attr( $option ); ?>_measure" data-option="<?php echo esc_attr( $option ); ?>"/>
...
if ( defined("OXYGEN_VSB_SHOW_BUILDER") ) {
	$toolbar = new OXYGEN_VSB_Toolbar();
}
